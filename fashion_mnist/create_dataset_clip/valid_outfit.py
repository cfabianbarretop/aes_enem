...
# CONFIG
# ==============================================
DATA_MNIST_FASHION_PATH = "dataset"  # Original dataset path
...
class MNISTFashionDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.mnist_dataset = torch.load(dataset_file)
        

        targets = np.array([sample["ground_truth"] for sample in self.mnist_dataset])
    ...
    def __getitem__(self, idx):
        ...
        s1 = self.mnist_dataset[i1]
        s2 = self.mnist_dataset[i2]
        s3 = self.mnist_dataset[i3]
        ...
        prob1 = s1["probability"]
        prob2 = s2["probability"]
        prob3 = s3["probability"]
    ...
    def collate_fn(batch):
        ...
        prob1 = torch.stack([item[3] for item in batch])
        prob2 = torch.stack([item[4] for item in batch])
        prob3 = torch.stack([item[5] for item in batch])
        digit1 = torch.tensor([item[6] for item in batch], dtype=torch.long)
        digit2 = torch.tensor([item[7] for item in batch], dtype=torch.long)
        digit3 = torch.tensor([item[8] for item in batch], dtype=torch.long)
        sum_3 = torch.tensor([item[9] for item in batch], dtype=torch.long)
        label = torch.tensor([item[10] for item in batch], dtype=torch.long)
        return (
            (img1, img2, img3),
            (prob1, prob2, prob3),
...
# Funcion data loader
# ==============================================
def mnist_fashion_loader(data_result, batch_size_train, batch_size_test):
    train_loader = torch.utils.data.DataLoader(
        MNISTFashionDataset(
            dataset_file=os.path.join(data_result, "fashion_clip_train.pt"),
            cache_file="fashion_clip_train.pkl",
    ...
    test_loader = torch.utils.data.DataLoader(
        MNISTFashionDataset(
            dataset_file=os.path.join(data_result, "fashion_clip_test.pt"),
            cache_file="fashion_clip_test.pkl",
            transform=mnist_img_transform,
        ),
        collate_fn=MNISTFashionDataset.collate_fn,
...
if __name__ == "__main__":
    ...
    data_dir = os.path.abspath(
        os.path.join(base_dir, DATA_RESULT_PATH, DATA_MNIST_FASHION_PATH)
    )
